...
const ROLES_VENTAS = ['asesor','supervisor','backoffice','validacion','grabaciones','seguimiento','jefatura','usuarios'];
...
router.post('/', auth(['asesor','backoffice','jefatura','usuarios']), (req, res) => {
  ...
  const result = db.prepare(`
    INSERT INTO ventas (
      ...
      predio, cuota_inst, claro_hogar, tecnologia, paquete,
  ...
  `).run(
    ...
    v.padre||null, v.madre||null, v.predio||null,
...
router.get('/', auth(ROLES_VENTAS), (req, res) => {
  const { dni, estado, desde, hasta, asesor_id } = req.query;
  ...
  if (hasta)  { sql += ` AND v.created_at <= ?`;   params.push(hasta+' 23:59:59'); }
...
router.patch('/:id', auth(ROLES_VENTAS), (req, res) => {
  const { estado, obs_backoffice, observacion } = req.body;
